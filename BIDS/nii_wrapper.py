...
from BIDS.core.nii_wrapper_math import NII_Math
...
Image_Reference = Union[BIDS.bids_files.BIDS_FILE, Nifti1Image, Path, str, N]
...
class NII(NII_Math):
    def __init__(self, nii: Nifti1Image, seg=False,c_val=None) -> None:
        ...
        self.__min = None 
    @classmethod
    def load(cls, path: Path | str, seg, c_val=None):
        return NII(nib.load(path), seg, c_val) #type: ignore
    @classmethod
    def load_bids(cls, nii: BIDS.bids_files.BIDS_FILE):
        ...
        if nii.get_interpolation_order() == 0:
            ...
            seg = False
            if 'ct' in nii.format.lower():
                c_val = -1024
            else:
                c_val = 0
        return NII(nib.load(nii.file['nii.gz']),seg,c_val=c_val) #type: ignore
    ...
    def get_c_val(self,default=None):
        ...
        if self.__min is None:
            self.__min = round(np.min(self.get_array()))
        return self.__min
    ...
    def set_array(self,arr:np.ndarray, inplace=False,verbose:logging=True)-> Self:
    ...
    def reorient(self:Self, axcodes_to: Ax_Codes = ("P", "I", "R"), verbose:logging=False, inplace=False)-> Self:
        ...
        new_img = Nifti1Image(arr, new_aff)
    ...
    def rescale(self, voxel_spacing=(1, 1, 1), c_val:float|None=None, verbose:logging=False, inplace=False,mode='constant'):
        """
    ...
    def resample_from_to(self, to_vox_map:Image_Reference|Proxy, mode='constant', c_val=None, inplace = False,verbose:logging=True):
    ...
    def n4_bias_field_correction_(self,mask=None,shrink_factor=4,convergence={"iters": [50, 50, 50, 50], "tol": 1e-07},spline_param=200,verbose=False,weight_mask=None,crop=False):
        return self.n4_bias_field_correction(mask=mask,shrink_factor=shrink_factor,convergence=convergence,spline_param=spline_param,verbose=verbose,weight_mask=weight_mask,crop=crop,inplace=True)
    ...
    def match_histograms(self, reference:Image_Reference,c_val = 0,inplace=False):
        ...
        ref = self.get_array()
        matched = match_histograms(ref, ref_nii.get_array())
    ...
    def erode_msk(self, mm: int = 5, connectivity: int = 3, inplace=False,verbose:logging=True):
        """
        ...
        for i in np.unique(msk_i_data):
            if i == 0:
                continue
            log.print(f"id = {i:03} erode mask",end='\r',verbose=verbose)
            data = msk_i_data.copy()
            data[i!=data] = 0
            mm2 = mm
            msk_ibe_data = binary_erosion(data, structure=struct, iterations=mm2)
    ...
    def dilate_msk(self, mm: int = 5, connectivity: int = 3, inplace=False, verbose:logging=True):
        """
        ...
        for i in np.unique(msk_i_data):
            if i == 0:
                continue
            log.print(f"id = {i:03} dilate mask",end='\r',verbose=verbose)
            data = msk_i_data.copy()
            data[i!=data] = 0
    ...
    def map_labels(self, label_map:Label_Map , verbose:logging=True, inplace=False):
        ...
        label_map_ = {
            (v_name2idx[k] if k in v_name2idx else int(k)): (v_name2idx[v] if v in v_name2idx else int(v))
            for k, v in label_map.items()
        }
        for v in np.unique(data):
            if v > 0 and v in label_map_:  # int needed to match non-integer data-types
                data[data_orig == v] = label_map_[v]
    ...
    def clone(self):
        return self.copy()
    def save(self,file:str|Path,make_parents=False,verbose:logging=True):
        if make_parents:
        ...
        log.print(f"Save {file} as {out.get_data_dtype()}",verbose=verbose,type=log_file.Log_Type.SAVE)
        nib.save(out, file) #type: ignore
    def __str__(self) -> str:
        return f"shp={self.shape}; ori={self.orientation}, zoom={self.zoom}, seg={self.seg}"
    @classmethod
    ...
    def remove_labels(self,*label:int, inplace=False):
        '''If this NII is a segmentation you can single out one label.'''
        assert label != 0, 'Zero label does not make sens.  This is the background'
        seg_arr = self.get_seg_array()
        for l in label:
...
def to_nii_seg(img: Image_Reference):
